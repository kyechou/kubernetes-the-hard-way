...
Copy the ca certificate to the worker nodes:

```
scp certs/ca.crt worker-2:~/
scp certs/ca.crt worker-3:~/
```
...
wget -q --show-progress --https-only --timestamping \
    ...
    "${DOWNLOAD_URL}/v1.18.9/bin/linux/amd64/kube-proxy" \
    "${DOWNLOAD_URL}/v1.18.9/bin/linux/amd64/kubelet"
```
...
sudo mkdir -p /etc/cni/net.d /opt/cni/bin \
    /var/lib/kubelet /var/lib/kube-proxy /var/lib/kubernetes /var/run/kubernetes
```
...
For the workers(kubelet) to access the Certificates API, they need to
authenticate to the kubernetes api-server first. For this we create a [Bootstrap
...
stringData:
  ...
  # Expiration. Optional.
  expiration: 2077-03-10T03:22:11Z
...
- **auth-extra-groups** - this is the group the worker nodes are part of. It must start with "system:bootstrappers:" The group does not exist yet. It is associated with this token.
...
This is to be done on the `worker-2` and `worker-3` nodes.

```
sudo kubectl config --kubeconfig=/var/lib/kubelet/bootstrap-kubeconfig set-cluster bootstrap --server='https://192.168.5.100:6443' --certificate-authority=/var/lib/kubernetes/ca.crt
...
> Remember to run the above commands on worker nodes: `worker-2` and `worker-3`.
...
worker-1   NotReady   <none>   80m    v1.18.9
worker-2   NotReady   <none>   100s   v1.18.9
worker-3   NotReady   <none>   100s   v1.18.9
```
